...
export class RuntimeValidator implements ValidationStage {
  ...
  private timeout = 60000; // 60s timeout
    ...
    try {
      ...
      if (!this.isDockerAvailable()) {
        return {
          ...
          errors: [],
          warnings: [{ code: 'W001', message: 'Docker not available, skipping runtime validation' }],
      ...
      if (!healthy) {
        ...
        return this.createResult(errors, warnings, startTime, container);
      }
      ...
      return {
        ...
        metrics: {
          ...
          avgResponseTime: results.reduce((sum, r) => sum + r.responseTime, 0) / totalCount
        },
        timeMs: Date.now() - startTime
      };
  ...
  async buildAndRun(workDir: string): Promise<ContainerInfo> {
    ...
    // Build
    execSync(`docker build -t ${imageName} -f ${workDir}/docker/Dockerfile.api ${workDir}/api`, {
      stdio: 'pipe',
      timeout: 120000
    });
