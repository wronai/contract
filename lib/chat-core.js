...
const rclUtils = require('./rcl-utils');
...
function normalizeMiniRclSource(source) {
  ...
  for (let raw of lines) {
    ...
    if (ctx !== 'enum') {
      ...
      // Also remove commas right before a closing brace (common JSON/TS style)
      // e.g. `version: "1.0.0", }` => `version: "1.0.0" }`
    ...
    // In app blocks, version must be a string literal
    if (ctx === 'app') {
      line = line.replace(/(\bversion\s*:\s*)(\d+\.\d+\.\d+)\b/g, '$1"$2"');
    }
    ...
    if (ctx === 'entity') {
      ...
      // Expand into multiple lines and normalize each field line.
      ...
      if (parts.length > 1) {
        for (const p of parts) {
          ...
          l = l.replace(/^(\s*)([A-Za-z_][A-Za-z0-9_]*)\s*:\s*/, '$1$2 ');
          ...
          l = l.replace(/@belongs_to\b/g, '');
          l = l.replace(/@has_many\b/g, '');

          // Remove legacy relation hints
          l = l.replace(/\(\s*(belongs_to|has_many)\s*\)/g, '');
...
class ReclappChat {
  ...
  /**
   * Call Ollama API
   * Delegates to lib/rcl-utils.js
   */
  async callOllama(messages) {
    return rclUtils.callOllama(messages, this.ollamaHost, this.model);
  }
  ...
  coerceToRclString(value) {
    return rclUtils.coerceToRclString(value);
  }

  isLikelyRcl(contract) {
    return rclUtils.isLikelyRcl(contract);
  }

  convertLegacyJsonContractToRcl(obj) {
    return rclUtils.convertLegacyJsonContractToRcl(obj);
  }

  extractContract(response) {
    return rclUtils.extractContract(response);
  }
  ...
  formatContract(contract) {
    if (!contract) return '';
    contract = normalizeMiniRclSource(contract);
    const lines = [];
    let indent = 0;
