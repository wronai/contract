...
${entity.fields.filter(f => !f.annotations.some(a => a.name === 'generated'))
  .map(f => `  ${f.name}${f.fieldType.nullable ? '?' : ''}: ${this.fieldTypeToTs(f.fieldType)};`).join('\n')}
}
...
${e.fields.map(f => `  ${f.name}: { type: ${this.fieldTypeToMongoose(f.fieldType)}, required: ${!f.fieldType.nullable} },`).join('\n')}
}, { timestamps: true });
